...
(function () {
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function (html) {
        ...
        */
        ...
        const firstImage =
          this.products.querySelector(".cart__image img") ||
          this.products.querySelector(".cart__image svg");
        if (
          firstImage &&
            ...
            function (cart) {
              ...
              if (
                ...
                if (updatedItem.quantity !== qty) {
                  alert(
                    theme.strings.maxQuantity
                      .replace("[quantity]", updatedItem.quantity)
  ...
  class ImageElement extends HTMLElement {
    constructor() {
      ...
      const handleIntersection = (entries, observer) => {
        ...
        if (imageWrap) {
          imageWrap.style.animation = "none";
        }

        // Find the img element and add the lazyloaded class if loading was set to lazy
        const imgEl = this.querySelector('img[loading="lazy"].lazyload');

        if (imgEl) {
          imgEl.classList.remove("lazyload");
          imgEl.classList.add("lazyloaded"); // to be used in styling for animations
        }
  ...
  theme.predictiveSearch = (function () {
    ...
    function predictQuery(q) {
      ...
      var requestResponse;
      var predictiveSearchSection =
        document.getElementById("PredictiveResults");
      var predictiveWrapper = document.getElementById("PredictiveWrapper");

      fetch(
        `${theme.routes.predictive_url}?${params}&section_id=predictive-search`
      )
        .then((response) => {
          isLoading = false;
    ...
    init() {
      fetch(this.url)
        ...
        */
            ...
            const firstImage =
              this.placeholder.querySelector(".grid-product__image-wrap img") ||
              this.placeholder.querySelector(".grid-product__image-wrap svg");
            if (
              firstImage &&
              firstImage.naturalWidth === 0 &&
              firstImage.naturalHeight === 0
  ...
  // Hide Judge.me preview badge if there are zero reviews (JS fallback for browsers without :has)
  DOMready(function () {
    try {
      var badges = document.querySelectorAll(".jdgm-widget.jdgm-preview-badge");
      badges.forEach(function (badge) {
        var prev = badge.querySelector(".jdgm-prev-badge");
        if (
          prev &&
          (prev.getAttribute("data-number-of-reviews") === "0" ||
            prev.getAttribute("data-number-of-reviews") === 0)
        ) {
